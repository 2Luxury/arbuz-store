document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    document.body.innerHTML =
      "<pre style='color:white'>JS ERROR:\n" + e + "</pre>";
  }
});
...
function init() {
  ...
  function renderShop() {
    ...
    app.innerHTML = `
      ...
      <div class="categories">
        ...
        ${cats.map(c=>`
          <div class="cat ${c===category?"active":""}"
               onclick="setCat('${c}', this)">
            ${c}
          </div>`).join("")}
      </div>
      ...
      <div class="grid">
        ${filtered.map(p=>`
          <div class="card">
            <div class="card-top">
              <b>${p.name}</b>
              <span class="heart ${fav.includes(p.id)?"active":""}"
                    onclick="toggleFav(${p.id})">❤️</span>
            </div>
  ...
  function renderFav() {
    ...
    app.innerHTML = "<h2>Избранное</h2>" +
      items.map(i=>`<p>${i.name}</p>`).join("");
  }

  function renderBottom() {
    bottom.innerHTML = `
      <div class="tab ${tab==="fav"?"active":""}"
           onclick="tab='fav';render()">❤️</div>
      <div class="tab ${tab==="game"?"active":""}"
           onclick="tab='game';render()">🎮</div>
      <div class="tab ${tab==="cart"?"active":""}"
           onclick="tab='cart';render()">🛒</div>
      <div class="tab ${tab==="profile"?"active":""}"
           onclick="tab='profile';render()">👤</div>
    `;
  }
  ...
  function moveLiquid(){
    ...
    liquid.style.left=active.offsetLeft+"px";
    liquid.style.top=active.offsetTop+"px";
  }
}
